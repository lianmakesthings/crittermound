...
describe.only('A map', () => {
  ...
  describe('on creation', () => {
    ...
    it('should generate special tiles', () => {
      const map = new Map(width, height);
      sinon.stub(map, 'generateBases');
      sinon.stub(map, 'generateTreasures');
      map.constructor();

      expect(map.generateBases).to.have.been.called;
      expect(map.generateTreasures).to.have.been.called;
    });
  });
  ...
  describe('generating artifacts', () => {
    ...
    it('should put artifact in specific range', () => {
      map.generateArtifacts();
      expect(map.randomInRange).to.always.have.been.calledWithExactly(0, 19);
    });
